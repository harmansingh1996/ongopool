...
export function registerAuthorizationRoutes(router: Router, client: PayPalClient, service: PayPalService) {
  router.post('/authorizations/:authorizationId/capture', async (req: Request, res: Response) => {
    try {
  ...
  router.post('/authorizations/:authorizationId/void', async (req: Request, res: Response) => {
    try {
      const { authorizationId } = req.params;
