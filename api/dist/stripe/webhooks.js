...
function toPlainMetadata(metadata) {
    if (!metadata)
        return undefined;
...
async function handleSetupIntentEvent(event) {
            ...
            default:
                console.debug('[Stripe][Webhook] Unhandled event type', event.type, {
                    object: event.data?.object && typeof event.data.object === 'object'
                        ? {
                            id: extractId(event.data.object.id),
                            metadata: toPlainMetadata(event.data.object.metadata),
                        }
                        : undefined,
                });
        }
