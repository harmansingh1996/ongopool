...
function serializePayoutMethod(record: PayoutMethodRecord | null | undefined) {
  if (!record) {
    return undefined;
  }
...
async function getPayoutMethodById(id: string): Promise<PayoutMethodRecord | null> {
...
async function getPayoutMethodsForUser(userId: string): Promise<PayoutMethodRecord[]> {
  const { data, error } = await supabase
    .from('payout_methods')
    .select(payoutMethodColumns)
    .eq('user_id', userId)
...
export interface ListPayoutMethodsResponse {
  success: boolean;
  data?: { payout_methods: ReturnType<typeof serializePayoutMethod>[] };
  error?: string;
  statusCode: number;
}
...
export interface SetDefaultPayoutMethodResponse {
  success: boolean;
  data?: { payout_method: ReturnType<typeof serializePayoutMethod> };
  error?: string;
  statusCode: number;
}
...
export async function setDefaultPayoutMethod(
  ...
  return updatedMethod as PayoutMethodRecord;
}
...
export async function deletePayoutMethod(
  payoutMethodId: string,
  bearerToken: string | null
): Promise<DeletePayoutMethodResponse> {
  try {
    // Validate input
    if (!payoutMethodId || typeof payoutMethodId !== 'string') {
      return {
...
function sendResponse(res: Response, result: { success: boolean; error?: string; statusCode: number }, data?: Record<string, unknown>) {
...
export function registerPayoutMethodRoutes(router: Router): void {
  /**
   ...
   */
  router.post('/payout-methods', async (req: Request, res: Response) => {
    try {
      const authHeader = req.headers.authorization;
      const bearerToken = extractBearerToken(authHeader);

      const body = req.body as CreatePayoutMethodRequest;
   ...
   */
  router.delete('/payout-methods/:id', async (req: Request, res: Response) => {
    try {
      const authHeader = req.headers.authorization;
      const bearerToken = extractBearerToken(authHeader);
   ...
   */
  router.get('/payout-methods', async (req: Request, res: Response) => {
    try {
      const authHeader = req.headers.authorization;
      const bearerToken = extractBearerToken(authHeader);
   ...
   * Response: 200 on success, 400/403/404/500 on error
   */
  router.post('/payout-methods/:id/default', async (req: Request, res: Response) => {
    try {
      const authHeader = req.headers.authorization;
      const bearerToken = extractBearerToken(authHeader);
      const payoutMethodId = req.params.id;
