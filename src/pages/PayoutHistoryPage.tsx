...
import { ChevronLeft, Clock, CheckCircle, XCircle, AlertCircle, DollarSign, Calendar, Filter, Download, Eye, Search, Loader2, Zap } from 'lucide-react';
...
import { stripeAPIClient } from '../lib/stripeApiClient';
...
const PayoutHistoryPage: React.FC = () => {
  ...
  const filteredPayouts = payoutRequests.filter((payout) => {
    ...
    const matchesSearch =
      normalizedSearch === '' ||
      payout.payment_method?.toLowerCase().includes(normalizedSearch) ||
      payout.status?.toLowerCase().includes(normalizedSearch) ||
      payout.amount.toString().includes(normalizedSearch) ||
      payout.payout_id?.toLowerCase().includes(normalizedSearch) ||
      payout.payroll_run_id?.toLowerCase().includes(normalizedSearch);
  ...
  const handleInitiatePayout = async (payout: PayoutHistoryData) => {
    try {
      setPayoutFeedback(null);
      setPayoutProcessingId(payout.id);

      const result = await stripeAPIClient.initiatePayout(payout.id);

      setPayoutRequests((prev) =>
        prev.map((existing) =>
          existing.id === payout.id
            ? {
                ...existing,
                status: 'processing',
                payout_id: result.payout_id,
                processed_at: result.arrival_date || new Date().toISOString(),
      ...
      const arrivalNote = result.arrival_date
        ? ` Expected arrival ${new Date(result.arrival_date).toLocaleDateString()}.`
        : ' Stripe will notify the connected driver once funds are on the way.';

      setPayoutFeedback({
        type: 'success',
        message: `Payout scheduled with Stripe. Reference ${result.payout_id}.${arrivalNote}`,
      });
    } catch (error) {
      ...
      setPayoutFeedback({
        ...
        message:
          error instanceof Error
            ? error.message
            : 'Unable to trigger payout. Please verify the Stripe account and try again.',
      });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        <div className="bg-white border border-blue-100 rounded-2xl shadow-sm p-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Stripe payout cadence</h2>
            <p className="mt-1 text-sm text-gray-600">
              Decide whether Stripe should send funds on a schedule or only when you click “Send payout”.
              Scheduled transfers are managed inside your Stripe Dashboard and apply to every connected driver.
            </p>
          </div>
          <div className="flex items-center gap-3">
            ...
            >
              ...
              Open Stripe payout schedule
            </a>
            <p className="text-xs text-gray-500">
              Tip: set a daily or weekly cadence for hands-free payouts. Manual transfers stay available below.
            </p>
          </div>
        </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    ...
                    {selectedPayout.payout_id && (
                      <div>
                        <label className="block text-sm font-medium text-gray-600">Stripe payout ID</label>
                        <p className="text-sm text-gray-900 break-all">{selectedPayout.payout_id}</p>
                      </div>
                    )}
                    {selectedPayout.payroll_run_id && (
                      <div>
                        <label className="block text-sm font-medium text-gray-600">Payroll run</label>
                        <p className="text-sm text-gray-900 break-all">{selectedPayout.payroll_run_id}</p>
                      </div>
                    )}
                    {selectedPayout.arrival_date && (
                      <div>
                        <label className="block text-sm font-medium text-gray-600">Estimated arrival</label>
                        <p className="text-sm text-gray-900">
                          {new Date(selectedPayout.arrival_date).toLocaleString()}
                        </p>
                      </div>
                    )}
                    <div>
