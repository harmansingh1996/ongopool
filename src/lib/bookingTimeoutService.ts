...
export class BookingTimeoutService {
   ...
   */
  private static async processExpiredHolds(): Promise<void> {
    try {
      console.log('Processing expired payment holds...');
      const result = await PaymentHoldService.releaseExpiredPaymentHolds(25);

      if (result.errors > 0) {
        console.error('Some expired holds failed to release automatically:', result);
      }

      if (result.processed > 0) {
        console.log(`Released ${result.processed} expired payment holds.`);
      }
    } catch (error) {
