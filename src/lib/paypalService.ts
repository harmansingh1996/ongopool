...
const resolvePayPalFunctionBase = () => {
  if (supabaseProjectRef) {
    return `https://${supabaseProjectRef}.functions.supabase.co/paypal-orders`;
  }

  if (supabaseRestUrl) {
    return `${supabaseRestUrl}/functions/v1/paypal-orders`;
  }

  console.warn('[PayPalService] Missing Supabase configuration. Falling back to relative function path.');
  return '/functions/v1/paypal-orders';
};

const PAYPAL_FUNCTION_BASE = resolvePayPalFunctionBase();
...
async function getAuthHeaders(): Promise<Record<string, string>> {
  ...
  try {
    ...
    if (error) {
      console.warn('[PayPalService] Failed to fetch session for auth header', error);
    }
    ...
    console.warn('[PayPalService] Unable to include auth header', error);
  }
...
async function requestPayPalBackend<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  const url = path.startsWith('http') ? path : `${PAYPAL_FUNCTION_BASE}${path}`;
...
export class PayPalService {
  ...
  static async createOrder(
    ...
    try {
      ...
      const order = await requestPayPalBackend<PayPalOrderData>('', {
        method: 'POST',
        body: JSON.stringify({
          action: 'createOrder',
          payload,
  ...
  static async captureOrder(orderId: string): Promise<PayPalOrderData> {
    try {
      const capturedOrder = await requestPayPalBackend<PayPalOrderData>('', {
        method: 'POST',
        body: JSON.stringify({
          action: 'captureOrder',
          orderId,
  ...
  static async getOrderDetails(orderId: string): Promise<PayPalOrderData> {
    try {
      const order = await requestPayPalBackend<PayPalOrderData>(`?action=getOrderDetails&orderId=${encodeURIComponent(orderId)}`, {
        method: 'GET',
      });
  ...
  static async authorizeOrder(orderId: string): Promise<PayPalAuthorizationData> {
    try {
      const authorization = await requestPayPalBackend<PayPalAuthorizationData>('', {
        method: 'POST',
        body: JSON.stringify({
          action: 'authorizeOrder',
          orderId,
        }),
      });
  ...
  static async captureAuthorization(authorizationId: string, amount?: number, currency: string = 'CAD'): Promise<any> {
    try {
      const capture = await requestPayPalBackend<any>('', {
        method: 'POST',
        body: JSON.stringify({
  ...
  static async voidAuthorization(authorizationId: string): Promise<void> {
    try {
      await requestPayPalBackend('', {
        method: 'POST',
        body: JSON.stringify({
          action: 'voidAuthorization',
          authorizationId,
        }),
      });
    } catch (error) {
